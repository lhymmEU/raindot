...
export async function GET() {
  ...
  try {
    ...
    const initiativeRes = await session.executeRead((tx) =>
      tx.run(initiativeQuery)
    );
    const functionalRes = await session.executeRead((tx) =>
      tx.run(functionalQuery)
    );

    // Transform the initiative data
    const rawInitiativeData = initiativeRes.records.map((record) =>
      record.toObject()
    );
    ...
    const rawFunctionalData = functionalRes.records.map((record) =>
      record.toObject()
    );
    ...
    console.log(
      `Treasury Outflow query completed successfully with ${initiativeData.length} initiative results and ${functionalData.length} functional results`
    );
    ...
    return NextResponse.json(
      {
        error: "Failed to fetch Treasury Outflow data",
